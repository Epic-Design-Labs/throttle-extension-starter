...
| Template customization       | `scripts/setup.test.ts`                                                                                                           | `pnpm run setup` rewrites only the allowlisted tokens, refuses unsafe input, and produces a type-checked provider skeleton when `--remove-demo` is used.                                                                                                                                                                                                                                                                    |
...
run setup -- --remove-demo` and write an equivalent lifecycle test against your
