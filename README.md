...
| `scripts/setup.mjs`                            | —                                    | The `pnpm run setup` template-customization command.                                                                                    |
...
Create `apps/extension-ui/.env.development.local` (git-ignored, and
mode-scoped so these dev-only values can never reach a production build — put
them here, **not** in `.env.local`, which Vite applies in every mode and would
ship the mock flag; see [docs/local-development.md](docs/local-development.md)):
...
| `VITE_USE_MOCK_BRIDGE`, `VITE_CONNECTOR_API_ORIGIN`, `VITE_THROTTLE_DASHBOARD_ORIGIN` | `apps/extension-ui/.env.development.local` (dev) / `.env.production` from `.env.production.example` (deploy) | **Browser-safe**                     | Vite inlines these into the client bundle at build time — never put an actual secret in a `VITE_` variable. Use mode-scoped files, not `.env.local` (which applies in every mode); a production build rejects dev-only mock flags and non-HTTPS origins. |
...
(Use `pnpm run setup`, not `pnpm setup` — the bare form invokes pnpm's own
built-in `setup` command, which modifies your shell profile instead of
running this script.)
